    ...
    <div className="mt-space-8">
      ...
      <div className="flex flex-wrap gap-space-2 mb-space-6">
        {MOVEMENT_DEFS.map(({ key, label, sub, verb }) => (
          ...
          >
            <span className="font-mono text-2xs uppercase tracking-widest mb-space-1" style={{ color: activeMovement === key ? `${BRICK}0.90)` : 'var(--color-neutral-500)' }}>
              {verb}
            </span>
            <span className="font-semibold text-sm leading-snug" style={{ color: activeMovement === key ? `${BRICK}0.90)` : 'var(--color-neutral-700)' }}>
              {label}
            </span>
            <span className="text-xs" style={{ color: activeMovement === key ? `${BRICK}0.90)` : 'var(--color-neutral-500)' }}>
              {sub}
            </span>
          </button>
        ))}
