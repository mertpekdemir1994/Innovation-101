        ...
        <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} width="100%" style={{ display: 'block', overflow: 'visible' }}>
          ...
          {STAGE_NAMES.map((name, i) => (
            <g key={name}>
              ...
              <text x={SCX[i]} y={HDR_TOP + HDR_H / 2 + 2}
                ...
                fill={`${TEAL}0.90)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
              >{name}</text>
            </g>
          ))}
          ...
          >
            ...
            <text x={4} y={CA_Y + 7} textAnchor="start" dominantBaseline="hanging"
              ...
              fill={`${TEAL}0.55)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
          ...
          >
            ...
            <text x={4} y={FS_Y + 7} textAnchor="start" dominantBaseline="hanging"
              fontSize="4.5" fontFamily="var(--font-mono)" letterSpacing="0.10em"
              fill={`${TEAL}0.45)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
            >FRONTSTAGE</text>
            {FS_ITEMS.map((text, i) => (
          ...
          >
            ...
            <text x={4} y={BS_Y + 7} textAnchor="start" dominantBaseline="hanging"
              fontSize="4.5" fontFamily="var(--font-mono)" letterSpacing="0.10em"
              fill={`${SLATE}0.58)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
            >BACKSTAGE</text>
            {BS_ITEMS.map((text, i) => (
              <text key={i} x={SCX[i]} y={BS_Y + BS_H / 2 + 5}
                textAnchor="middle" dominantBaseline="middle"
                fontSize="6.5" fontFamily="var(--font-body, Inter, sans-serif)"
                fill={`${SLATE}0.72)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
              >{text}</text>
            ))}
          </motion.g>
          ...
          >
            ...
            <text x={4} y={SP_Y + 7} textAnchor="start" dominantBaseline="hanging"
              fontSize="4.5" fontFamily="var(--font-mono)" letterSpacing="0.10em"
              fill={`${SLATE}0.48)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
            >SUPPORT PROCESSES / SYSTEMS</text>
            {SP_ITEMS.map((text, i) => (
              <text key={i} x={SCX[i]} y={SP_Y + SP_H / 2 + 5}
                textAnchor="middle" dominantBaseline="middle"
                fontSize="6.5" fontFamily="var(--font-body, Inter, sans-serif)"
                fill={`${SLATE}0.62)`} style={{ userSelect: 'none', pointerEvents: 'none' }}
              >{text}</text>
            ))}
          </motion.g>
        </svg>
