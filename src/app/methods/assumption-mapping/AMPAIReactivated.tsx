...
// Cards stack single-file within their quadrant so each has room for its
// label at a legible size.
const LOF_CARD = { x: 380, y: 90, w: 252, label: 'WILL THEY BUY ONLINE?' }
const TRAP_CARD = { x: 104, y: 126, w: 252, label: 'WILL THEY BUY ONLINE?' }

const STATIC_CARDS = [
  { id: 's1', x: 104, y: 54,  w: 252, label: 'BEHAVIOUR EXISTS' },
  { id: 's2', x: 104, y: 90,  w: 252, label: 'TECH IS READY' },
  { id: 's3', x: 380, y: 54,  w: 252, label: 'PRICE ACCEPTED?' },
  { id: 's4', x: 380, y: 178, w: 252, label: 'PREFER FREE RETURNS' },
  { id: 's5', x: 104, y: 178, w: 252, label: 'CAN SHIP PRODUCT' },
  { id: 's6', x: 104, y: 214, w: 252, label: 'CAN BUILD SITE' },
]
    ...
    <div className="w-full">
      ...
      >
        ...
        <text x={GX + 8} y={GY + 15} textAnchor="start"
          ...
          fill={isAI ? `${INDIGO_TEXT}0.905)` : 'rgba(255,255,255,0.62)'}
        ...
        <text x={GR - 8} y={GY + 15} textAnchor="end"
          ...
          fill={isAI ? 'rgba(255,255,255,0.60)' : `${CLAY_TEXT}0.958)`}
          style={{ userSelect: 'none', transition: 'fill 0.4s' }}>
          {isAI ? 'LEAP OF FAITH' : 'LEAP OF FAITH · TEST FIRST'}
        </text>
        <text x={GX + 8} y={GB - 9} textAnchor="start"
          fontSize="11" fontFamily="var(--font-mono)" letterSpacing="0.08em"
          fill="rgba(255,255,255,0.58)" style={{ userSelect: 'none' }}>IGNORE</text>
        <text x={GR - 8} y={GB - 9} textAnchor="end"
          fontSize="11" fontFamily="var(--font-mono)" letterSpacing="0.06em"
          fill="rgba(255,255,255,0.60)" style={{ userSelect: 'none' }}>NICE TO KNOW</text>
        ...
        {STATIC_CARDS.filter(c => !(isAI && c.id === 's1' && false)).map(c => (
          <g key={c.id}>
            ...
            <text x={c.x + c.w / 2} y={c.y + CARD_H / 2 + 1}
              ...
              fill="rgba(255,255,255,0.68)" style={{ userSelect: 'none' }}>{c.label}</text>
          </g>
        ))}
        ...
        <AnimatePresence>
          {isAI && (
            ...
            >
              ...
              <line x1={360} y1={155} x2={390} y2={155}
                stroke={`${INDIGO}0.45)`} strokeWidth={1}
                strokeDasharray="3 2" strokeLinecap="round" />
              <path d={`M 385 150 L 392 155 L 385 160`}
                stroke={`${INDIGO}0.45)`} strokeWidth={1} fill="none"
                strokeLinecap="round" strokeLinejoin="round" />
              {/* Trap badge — in the gap between quadrants, clear of both card columns */}
              <rect x={358} y={122} width={140} height={20} rx={3}
                fill={`${INDIGO}0.14)`} stroke={`${INDIGO}0.45)`} strokeWidth={1} />
              <text x={428} y={132} textAnchor="middle" dominantBaseline="middle"
                fontSize="11" fontFamily="var(--font-mono)" letterSpacing="0.03em"
          ...
          >
            {[
              ...
              <div key={i} className="rounded-lg p-4 border"
                ...
                <p className="text-[10px] font-semibold uppercase tracking-wider mb-2"
                  style={{ color: `${CLAY_TEXT}0.95)` }}>{card.h}</p>
                <p className="text-xs text-white/55 leading-relaxed">{card.b}</p>
              </div>
            ))}
          </motion.div>
